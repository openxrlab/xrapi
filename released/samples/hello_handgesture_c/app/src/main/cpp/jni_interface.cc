...
#define JNI_METHOD(return_type, method_name) \
  JNIEXPORT return_type JNICALL              \
      Java_com_standardar_examples_c_handgesture_JniInterface_##method_name
...
(JNIEnv *env, jclass , jobject j_asset_manager, jobject context, jstring j_package_name) {
    ...
    HelloArActivityClaz = env->FindClass("com/standardar/examples/c/handgesture/HelloArActivity");
    updateHandGestureInfoMid = env->GetMethodID(HelloArActivityClaz,"updateHandGestureInfo","(Ljava/lang/String;[FIIIIII)V");
    s_helloArActivityObject = env->NewGlobalRef(context);
...
jclass getHelloArActivity(){
    return HelloArActivityClaz;
}

jobject getHelloArActivityObj(){
    return s_helloArActivityObject;
}

jmethodID getUpdateHandGestureInfoMid(){
    return updateHandGestureInfoMid;
}
