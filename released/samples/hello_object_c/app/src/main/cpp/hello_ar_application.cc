...
#define MS_PER_US     1000
            ...
            : asset_manager_(asset_manager)
    {
        ...
        arConfigSetAlgorithmMode(ar_config, ARALGORITHM_TYPE_OBJECT_SCANNING, ARALGORITHM_MODE_ENABLE);
        //arConfigSetAlgorithmMode(ar_config, ARALGORITHM_TYPE_ILLUMINATION_ESTIMATION, ARALGORITHM_MODE_ENABLE);
        arConfigSetPlaneDetectingMode(ar_config, ARPLANE_DETECTING_MODE_HORIZONTAL);
    ...
    void HelloArApplication::OnPause() {
        ...
        m_bFirstPlace = true;
        m_EnableScan = false;
        scanner_renderer_.StopScanGuidance();
        scanner_renderer_.Clear();
        m_bShowPlane = true;
        m_BoxMat = glm::mat4();
        m_ScaleMat[0][0] = 0.1f;
        m_ScaleMat[1][1] = 0.1f;
    ...
    void HelloArApplication::OnDrawFrame() {
        ...
        scanner_renderer_.SetFov(verticalFOV, (float)m_ViewportWidth / m_ViewportHeight);
        ...
        if(!m_bFirstPlace) {
            scanner_renderer_.Draw(projection_mat, view_mat, m_BoxMat * m_ScaleMat, light_intensity);
        }
        ...
        if(m_bFinishScanning) {
            point_cloud_renderer_.DrawObjectPointCloud(projection_mat * view_mat * m_BoxMat, ar_session_);
        }
    ...
    void HelloArApplication::OnTouched(float x, float y) {
        ...
        if (ar_frame_ != nullptr && ar_session_ != nullptr) {
            ...
            for (int32_t i = 0; i < query_result_list_size; ++i) {
                ...
                if(!m_bFirstPlace) {
                    float normalizex = x / (float) m_ViewportWidth;
                    float normalizey = y / (float) m_ViewportHeight;
                    scanner_renderer_.OnSingleTouched(normalizex, normalizey);
                    return;
                }

                util::GetTransformMatrixFromAnchor(ar_session_, anchor, &m_BoxMat);
                m_bFirstPlace = false;
    ...
    void HelloArApplication::OnDoubleTouched(float x, float y)
    {
        if (!m_EnableScan)
            return;
        ...
        float normalizex = x / (float) m_ViewportWidth;
        float normalizey = y / (float) m_ViewportHeight;
        scanner_renderer_.OnDoubleTouched(normalizex, normalizey);
    }

    void HelloArApplication::OnTouchedZoom(float scale){
        if (!m_EnableScan)
            return;

        m_ScaleMat[0][0] *= scale;
        m_ScaleMat[1][1] *= scale;
        m_ScaleMat[2][2] *= scale;
    }

    void HelloArApplication::OnTouchedMove(float x, float y){
        if (!m_EnableScan)
            return;
        ...
        if (ar_frame_ != nullptr && ar_session_ != nullptr) {
            ...
            for (int32_t i = 0; i < query_result_list_size; ++i) {
                ...
                glm::mat4 model_mat;
                util::GetTransformMatrixFromAnchor(ar_session_, anchor, &model_mat);

                float normalizex = x / (float) m_ViewportWidth;
                float normalizey = y / (float) m_ViewportHeight;
                scanner_renderer_.OnTouchedMove(ar_session_, ar_map_, normalizex, normalizey, m_ScaleMat, m_BoxMat);
                //scanner_renderer_.OnTouchedMove(m_insectMat, model_mat, m_ScaleMat, m_BoxMat);
                arQueryResultDestroy(ar_query_result);
                ar_query_result = nullptr;
            }
    ...
    void HelloArApplication::ResetTouched()
    {
        scanner_renderer_.ResetTouched(m_BoxMat);
    }
    ...
    const char* HelloArApplication::getDebugInfo(){
        ...
        char slam_version[1024];
        arSessionGetVersion(ar_session_, slam_version, 1024);

        //LOGI("progress %i, version %s", progress, slam_version.c_str());
    ...
    bool HelloArApplication::IsScanStart()
    {
        return !m_bFirstPlace;
    }

    void HelloArApplication::OnStartScan() {
        m_EnableScan = true;
        m_bShowPlane = true;
        m_bFirstPlace = true;
        m_bFinishScanning = false;
    ...
    void HelloArApplication::OnStopScan() {
        m_EnableScan = false;
        scanner_renderer_.Clear();
    }

    void HelloArApplication::OnStartScanGuidance() {
        float vertex[24];
        scanner_renderer_.GetBoundingBox(vertex);
        arSessionSetBoundingBox(ar_session_, vertex, glm::value_ptr(m_BoxMat * m_ScaleMat));
        scanner_renderer_.StartScanGuidance();
        m_bShowPlane = false;
    }

    void HelloArApplication::OnStopScanGuidance() {
        scanner_renderer_.StopScanGuidance();
        m_EnableScan = false;

    }

    void HelloArApplication::ResetAlgorithm() {
        ...
        m_ScaleMat[1][1] = 0.1f;
        m_ScaleMat[2][2] = 0.1f;
        arSessionStartAlgorithm(ar_session_, ARALGORITHM_TYPE_SLAM, 0);
    }

    void HelloArApplication::getScanResult() {
        uint8_t *result;
        int64_t result_size = 0;
        arSessionGetBoundingBoxScanningResult(ar_session_, &result, &result_size);
        if(result_size !=0)
            util::SaveObjectPointCloud("/sdcard/1.sot", result, result_size);
        else
            LOGI("Scanning result is not valid!");
        m_bFinishScanning = true;
    }

    int HelloArApplication::getScanProgress() {
        int progress = scanner_renderer_.GetScanProgress();
        return progress;
    }

    bool HelloArApplication::isNearPlane(){
        return scanner_renderer_.IsNearPlane();
    }
