...
constexpr char kJniInterfaceClassName[] =
        "com/standardar/examples/c/object/ObjectTrackingJniInterface";
constexpr char kHideFitToScanMethodName[] = "hideFitToScanImage";
constexpr char kHideFitToScanMethodSignature[] =
        "(Lcom/standardar/examples/c/object/ObjectTrackingActivity;)V";
...
bool LoadFile(const std::string& path, char** out_file_buffer, int* file_buffer_length)
{
    ...
    {
        succ = FileUtils::readFile(path, out_file_buffer, file_buffer_length);
    }
...
bool LoadPngFromAssetManager(int target, const std::string& path) {
  ...
  static struct JNIData {
    ...
    constexpr char kHelperClassName[] =
        "com/standardar/examples/c/object/JniInterface";
...
void SaveObjectPointCloud(std::string outputPath, uint8_t *raw_bytes, int64_t raw_bytes_size) {
  ...
  if (out == nullptr)
  {
    LOGE("open file %s failed! %d", outputPath.c_str(), errno);
    return;
  }

  fwrite(raw_bytes, raw_bytes_size, 1, out);
  fclose(out);
}
