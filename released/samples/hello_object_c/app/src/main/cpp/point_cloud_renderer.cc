...
void PointCloudRenderer::InitializeGlContent() {
  ...
  uniform_color_vec4 = glGetUniformLocation(shader_program_, "color");
...
void PointCloudRenderer::Draw(glm::mat4 mvp_matrix, ARSession* ar_session,
  ...
  glUniform4f(uniform_color_vec4, 0.1215f, 0.7372f, 0.8235f, 1.0f);
...
void PointCloudRenderer::DrawObjectPointCloud(glm::mat4 mvp_matrix, ARSession* ar_session) const {
  CHECK(shader_program_);
  ...
  arSessionGetBoundingBoxPointCloud(ar_session, &point_cloud_data, &number_of_points);
  if(!number_of_points)
    return;

  glUniform4f(uniform_color_vec4, 1.0f, 0.0f, 0.0f, 1.0f);
  glUniformMatrix4fv(uniform_mvp_mat_, 1, GL_FALSE, glm::value_ptr(mvp_matrix));
