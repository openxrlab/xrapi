...
constexpr char kVertexShader_TOF[] = R"(
...
void main() {
    ...
    v_ClipPos = u_ModelViewProjection * a_Position;
    gl_Position = v_ClipPos;
}
)";

constexpr char kFragmentShader_TOF[] = R"(
...
void main() {
    ...
    vec3 projCoords = v_ClipPos.xyz / v_ClipPos.w;
    projCoords = projCoords *0.5 + 0.5;

    float temp = projCoords.x;
    projCoords.x = 1.0 - projCoords.y;
    projCoords.y = 1.0 - temp;

    float depth = texture2D(u_depthTexture, projCoords.xy).r;
    depth *= u_Far;
    float currentDepth = - v_ViewPosition.z;
    currentDepth *= 1000.0;
...
void ObjRenderer::InitializeGLContext(bool bUseTOF)
{
    bEnableTOF = bUseTOF;
    if (bUseTOF)
    {
        shader_program_ = util::CreateProgram(kVertexShader_TOF, kFragmentShader_TOF);
    }
    else
    {
    ...
    if (bUseTOF)
    {
        uniform_depthTexture_ = glGetUniformLocation(shader_program_, "u_depthTexture");
        uniform_far_ = glGetUniformLocation(shader_program_, "u_Far");
    }
}

void ObjRenderer::InitalizeGLDepthTexture(int width, int height)
{
    m_depthWidth = width;
    m_depthHeight = height;
    glGenTextures(1, &depthTexture_id_);
...
void ObjRenderer::InitializeResources(AAssetManager* asset_manager,
...
void ObjRenderer::UpdateDepthImage(uint16_t * image)
{
    glBindTexture(GL_TEXTURE_2D, depthTexture_id_);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_CLAMP_TO_EDGE);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_CLAMP_TO_EDGE);
    glTexSubImage2D(GL_TEXTURE_2D, 0, 0, 0, m_depthWidth, m_depthHeight, GL_DEPTH_COMPONENT, GL_UNSIGNED_SHORT, image);
}
...
void ObjRenderer::Draw(const glm::mat4& projection_mat,
    ...
    if (bEnableTOF) {
        ...
        glUniform1i(uniform_depthTexture_, 1);
        glBindTexture(GL_TEXTURE_2D, depthTexture_id_);
        glUniform1f(uniform_far_, farClip);
    }
