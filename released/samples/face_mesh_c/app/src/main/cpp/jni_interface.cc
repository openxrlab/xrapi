...
#define JNI_METHOD(return_type, method_name) \
  JNIEXPORT return_type JNICALL              \
      Java_com_standardar_examples_c_facemesh_JniInterface_##method_name
...
JNI_METHOD(void, setTexture)
(JNIEnv * env, jclass, jlong native_application,jobject bitmap) {
    native(native_application)->setTexture(env,bitmap);
}

JNI_METHOD(void, switchCamera)
(JNIEnv *, jclass, jlong native_application) {
    native(native_application)->switchCamera();
}
