...
constexpr char kJniInterfaceClassName[] =
    "com/standardar/ar/core/examples/c/augmentedimage/JniInterface";
constexpr char kLoadImageMethodName[] = "loadImage";
constexpr char kLoadImageMethodSignature[] =
    "(Ljava/lang/String;)Landroid/graphics/Bitmap;";
constexpr char kConvertToGrayScaleMethodName[] = "convertBitmapToGrayscaleDirectBuffer";
constexpr char kConvertToGrayScaleMethodSignature[] =
...
constexpr char kHideFitToScanMethodSignature[] =
    "(Lcom/standardar/ar/core/examples/c/augmentedimage/AugmentedImageActivity;)V";

static jclass jni_class_id = nullptr;
static jmethodID jni_load_image_method_id = nullptr;
static jmethodID jni_convert_to_grayscale_method_id = nullptr;
...
void InitializeJavaMethodIDs() {
  ...
  jni_convert_to_grayscale_method_id =
      env->GetStaticMethodID(jni_class_id,
                             kConvertToGrayScaleMethodName,
                             kConvertToGrayScaleMethodSignature);
...
void ReleaseJavaMethodIDs() {
  ...
  jni_convert_to_grayscale_method_id = nullptr;
...
static jobject CallJavaConvertToGrayScale(jobject image) {
    JNIEnv* env = GetJniEnv();
    return env->CallStaticObjectMethod(jni_class_id, jni_convert_to_grayscale_method_id,
                                       image);
}
...
bool extractAssetFile(AAssetManager* mgr, const std::string& file_name, const std::string& apk_internal_path,
  ...
  std::string file_path = apk_internal_path + "/" + file_name;
  bool succ = false;
  if (access(file_path.c_str(), F_OK) == 0)
  {
    succ = FileUtils::readFile(file_path, out_file_buffer, file_buffer_length);
  }
  else
  {
    succ = FileUtils::extractAsset(mgr, file_name, file_path);
    if (!succ)
    {
      return false;
    }
...
bool LoadImageFromAssetManager(const std::string& path, int* out_width,
  ...
  *out_stride = bitmap_info.width;

  jobject buffer = CallJavaConvertToGrayScale(image_obj);
  int64_t total_size_in_byte = env->GetDirectBufferCapacity(buffer);
  uint8_t *buffer_ptr = (uint8_t*) env->GetDirectBufferAddress(buffer);
  *out_pixel_buffer = new uint8_t[total_size_in_byte];
  memcpy(*out_pixel_buffer, buffer_ptr, total_size_in_byte);
...
void GetTransformMatrixFromAnchor(ARSession* ar_session,
  ...
  arPoseGetMatrix(pose.GetArPose(),
